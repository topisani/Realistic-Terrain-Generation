...
public final class BiomeAnalyzer {
  ...
  private Biome scenicLakeBiome = Biomes.RIVER;
  private Biome scenicFrozenLakeBiome = Biomes.FROZEN_RIVER;
  ...
  public static final int[] XYINVERTER = xyinverted();
  ...
  private void initBiomes() {
    ...
    Registry.BIOME.stream().forEach(biome -> {
      ...
      int id = WorldUtil.Biomes.getBiomeID(biome);
  ...
  private void setupBeachesForBiomes() {
    ...
    for (int i = 0; i < preferredBeach.length; i++) {
      ...
      // We need to work with the rtg biome, so let's try to get it from the base biome, aborting if necessary.
      Biome biome = Registry.BIOME.getByValue(i);
      if (biome == null) {
        continue;
      }

      preferredBeach[i] = WorldUtil.Biomes.getBiomeID(getBeachFor(biome));
    }
  }

  public void newRepair(final Biome[] genLayerBiomes, final int[] biomeNeighborhood, final ChunkLandscape landscape) {

    final Biome[] jitteredBiomes = landscape.biomes;
    final float[] noise = landscape.noise;
    final float[] riverStrength = landscape.river;

    IRTGBiome realisticBiome;
    Biome biome;
    ...
    for (int i = 0; i < MAX_BIOMES; i++) {
      ...
      biome = genLayerBiomes[i];
      int biomeID = WorldUtil.Biomes.getBiomeID(biome);
      ...
      if (noise[i] > 61.5) {
        ...
        // check for river
        if (canBeRiver && !oceanBiome[biomeID] && !swampBiome[biomeID]) {
          // make river
          jitteredBiomes[i] = getRiverFor(biome);
        } else {
          // replace
          jitteredBiomes[i] = biome;
        }
    ...
    for (int i = 0; i < MAX_BIOMES; i++) {
      ...
      int biomeID = WorldUtil.Biomes.getBiomeID(jitteredBiomes[i]);
      ...
      if (foundBiome != NO_BIOME) {
        ...
        jitteredBiomes[i] = WorldUtil.Biomes.getById(foundBiome);
      }
    }
    ...
    for (int i = 0; i < MAX_BIOMES; i++) {
      ...
      if (noise[i] <= 61.5 && !riverBiome[biomeID]) {
        ...
        if (!oceanBiome[biomeID] && !swampBiome[biomeID] && !beachBiome[biomeID]) {
          Biome riverReplacement = getRiverFor(jitteredBiomes[i]); // make river
          if (riverReplacement == Biomes.FROZEN_RIVER) {
  ...
  private Biome getRiverFor(Biome biome) {
    IRTGBiome rtgBiome = RTGAPI.getRTGBiome(biome);
    if (rtgBiome != null && rtgBiome.getRiverBiome() != null) {
      return rtgBiome.getRiverBiome().baseBiome();
    }
    // TODO: Heuristics to pick a river
    return Biomes.RIVER;
  }

  private Biome getBeachFor(Biome biome) {
    IRTGBiome rtgBiome = RTGAPI.getRTGBiome(biome);
    if (rtgBiome != null && rtgBiome.getBeachBiome() != null) {
      return rtgBiome.getBeachBiome().baseBiome();
    }
    // TODO: Heuristics to pick a beach
    return Biomes.BEACH;
  }
