...
public final class BiomeMap implements IntegerLIMap<Biome, IRTGBiome> {

    private static final long serialVersionUID = -5276712268351774212L;

    @SuppressWarnings("unchecked")
    transient private final IndexMapEntry<Biome, IRTGBiome, Integer>[] entries = new IndexMapEntry[getCapacity()];
    ...
    public void addBiomes(IRTGBiome... rbiomes) {
    ...
    @Override
    @Nullable
    public IRTGBiome getValueAt(@Nonnull final Integer index) {
        if (checkBounds(index)) {
            return null;
        }
    ...
    @Override
    @Nullable
    public IRTGBiome putAt(@Nonnull final Biome key, @Nonnull final IRTGBiome value, final Integer index) {
        if (locked) {
            return null;
        }
        if (checkBounds(index)) {
    ...
    @Override
    @Nullable
    public IRTGBiome remove(@Nonnull final Integer index) {
        if (locked) {
            return null;
        }
        IRTGBiome oldValue = getValueAt(index);
        entries[index] = null;
        return oldValue;
    }
    ...
    @Nullable
    private IRTGBiome insert(@Nonnull final IndexMapEntry<Biome, IRTGBiome, Integer> newEntry, final int index) {
        Entry<Biome, IRTGBiome> oldValue = entries[index];
    ...
    @Override
    @Nullable
    public IRTGBiome get(@Nonnull final Object key) {
        return Arrays.stream(entries).filter(e -> Objects.nonNull(e) && e.getKey().equals(key)).findFirst().map(Entry::getValue).orElse(null);
    }

    @Override
    @Nullable
    public IRTGBiome put(@Nonnull final Biome key, @Nonnull final IRTGBiome value) {
        if (locked) {
            return null;
        }
    ...
    @Override
    @Nullable
    public IRTGBiome remove(@Nonnull final Object key) {
        if (locked) {
            return null;
        }
    ...
    @Override
    public void putAll(@Nonnull final Map<? extends Biome, ? extends IRTGBiome> m) {
        if (locked) {
            return;
        }
        m.forEach(this::put);
    ...
    public String toString() {
        ...
        boolean first = true;
        for (Entry<Biome, IRTGBiome> e : entrySet()) {
            if (first) {
                ret.append("{");
            }
            else {
    ...
    public Stream<Entry<Biome, IRTGBiome>> stream() {
        return StreamSupport.stream(Spliterators.spliterator(entrySet(), 0), false);
    }

    private IndexMapEntry<Biome, IRTGBiome, Integer> getNewEntry(@Nonnull Biome key, @Nonnull IRTGBiome value) {
