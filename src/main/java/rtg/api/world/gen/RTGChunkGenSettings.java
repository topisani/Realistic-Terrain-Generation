...
public final class RTGChunkGenSettings extends GenerationSettings {
...
//          this.snowDuneHeight         = 4;
    ...
    public static class Serializer implements JsonDeserializer<Factory>, JsonSerializer<Factory> {
        ...
        static class JsonUtils {
            ...
            static boolean getBoolean(JsonObject json, String key, boolean def) { return json.has(key) ? json.get(key).getAsBoolean() : def; }
            static float getFloat(JsonObject json, String key, float def) { return json.has(key) ? json.get(key).getAsFloat() : def; }
        }
