...
public abstract class RTGBiomeBase implements IRTGBiome {
  ...
  public RTGBiomeBase(@Nonnull final Biome baseBiome) {
    this(baseBiome, RiverType.NORMAL, BeachType.NORMAL);
  }

  public RTGBiomeBase(@Nonnull final Biome baseBiome, @Nonnull final RiverType riverType) {
    this(baseBiome, riverType, BeachType.NORMAL);
  }

  public RTGBiomeBase(@Nonnull final Biome baseBiome, @Nonnull final BeachType beachType) {
    this(baseBiome, RiverType.NORMAL, beachType);
  }

  public RTGBiomeBase(@Nonnull final Biome baseBiome, @Nonnull final RiverType riverType, @Nonnull final BeachType beachType) {
    ...
    if (resloc == null) {
      throw new IllegalStateException(String.format("Biome with ID: %s, of class: %s, does not have a registry name set.",
          Registry.BIOME.getId(baseBiome), baseBiome.getClass().getName()));
    }

    this.baseBiome = baseBiome;
    this.baseBiomeResLoc = resloc;
    this.baseBiomeId = WorldUtil.Biomes.getBiomeID(baseBiome);
    this.riverType = riverType;
    this.beachType = beachType;
    this.config = new BiomeConfig(); // TODO: new BiomeConfig(getConfigFile());
  ...
  @Override
  public IRTGBiome getRiverBiome() {
    return this.riverType.getRTGBiome();
  }

  @Override
  public IRTGBiome getBeachBiome() {
    IRTGBiome rbb = RTGAPI.getRTGBiome(WorldUtil.Biomes.getBiomeID(this.preferredBeach()));
  ...
  @Override
  public void rReplace(RTGChunkWrapper chunk, BlockPos blockPos, int x, int y, int depth, RTGWorld rtgWorld, float[] noise, float river, Biome[] base) {
    rReplace(chunk, blockPos.getX(), blockPos.getZ(), x, y, depth, rtgWorld, noise, river, base);
  }
  ...
  public void rReplace(RTGChunkWrapper chunk, int i, int j, int x, int y, int depth, RTGWorld rtgWorld, float[] noise, float river, Biome[] base) {
    ...
    this.surface.paintTerrain(chunk, i, j, x, y, depth, rtgWorld, noise, riverRegion, base);
  }

  protected void rReplaceWithRiver(RTGChunkWrapper chunk, int i, int j, int x, int y, int depth, RTGWorld rtgWorld, float[] noise, float river, Biome[] base) {
    if (RTG.surfacesDisabled() || this.getConfig().DISABLE_RTG_SURFACES.get()) {
      return;
    }
    float riverRegion = !this.getConfig().ALLOW_RIVERS.get() ? 0f : river;
    ...
    if (RTGConfig.lushRiverbanksInDesert()) {
      this.surfaceRiver.paintTerrain(chunk, i, j, x, y, depth, rtgWorld, noise, riverRegion, base);
    }
  }
  ...
  protected BeachType determineBeachType() {
    ...
    //float height = baseBiome() + (baseBiome().getHeightVariation() * 2f);
  ...
  public enum BeachType {
    ...
    private IRTGBiome rtgBiome;
    private boolean locked = false;

    public Biome getBiome() {
      return (this == STONE) ? Biomes.STONE_SHORE: ((this == COLD) ? Biomes.SNOWY_BEACH : Biomes.BEACH);
    }

    public IRTGBiome getRTGBiome() {
      return rtgBiome;
    }

    public IRTGBiome setRTGBiome(IRTGBiome rtgBiome) {
      if (!locked) {
    ...
    public BeachType getTypeFromBiome(Biome beachBiome) {
      return (beachBiome == Biomes.STONE_SHORE) ? STONE : ((beachBiome == Biomes.SNOWY_BEACH) ? COLD : NORMAL);
    }
  }
